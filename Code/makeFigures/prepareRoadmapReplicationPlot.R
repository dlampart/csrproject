...
plottingTable=rbind(plottingTable,fun(100,"rank < 100",noinv=TRUE,alldf1SubFam,alldf1SubFamRM))
plottingTable=rbind(plottingTable,fun(100,"rank >= 100",noinv=FALSE,alldf1SubFam,alldf1SubFamRM))
plottingTable

all_labels=unique(plottingTable[,pioneers])
plottingTable[,pioneers:=factor(pioneers,levels=all_labels)]
#colors=c("000","000","000","000","000")
colors=paste("grey", floor(seq(from=1,to=70,length.out=length(all_labels))),sep="")
p=ggplot()
...
p=p + ylim(c(0,1)) + ylab("cumulative distribution") + xlab("subfamily rank") + theme_classic()+theme(axis.title=element_text(size=18,face="bold"),axis.text=element_text(size=16),legend.title=element_blank(),legend.text=element_text(size=16),legend.position=c(0.75,0.25))+ geom_abline(slope=1/maxRank )+ theme(axis.line.x = element_line(colour = 'black', size=0.5, linetype='solid'),axis.line.y = element_line(colour = 'black', size=0.5, linetype='solid'))
print(p)
#browser()
