...
p=ggplot(data=tabl2, aes(x=-log10((theoretical)),y=-log10((controlledlm1))))+geom_point(size=3,colour="blue")+geom_point(data=tabl2[is.element(names,ww),], size=3,colour="red")+geom_abline(slope=1,intercept=0)+geom_text(data=tabl2[is.element(names,ww2),],aes(label=names),hjust=1, vjust=-0.2,size=7)+xlab("-log10 p-values theoretical")+ylab("-log10 p-values empirical")+theme_classic()+theme(axis.title=element_text(size=18,face="bold"),axis.text=element_text(size=16))+ geom_text(data=tabl2[is.element(names,ww),],aes(label=names),hjust=1, vjust=-0.2,size=7)
plot(p)
dev.off()
