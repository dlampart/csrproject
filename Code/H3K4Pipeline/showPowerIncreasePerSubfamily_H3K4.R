...
p=ggplot(data=cumSumTable[cum<55,],aes(x=rank,y=probability,group=Method,colour=Method))+geom_line(size=3) + coord_cartesian(xlim=c(0,30))+geom_abline(slope=a2/b1) + scale_y_continuous(labels = scales::percent,expand=c(0,0))+ theme_classic() + ylim(0,0.45)+theme(axis.text=element_text(size=18), axis.title=element_text(size=18,face="bold"),legend.title=element_text(size=18,face="bold"),legend.text=element_text(size=18),legend.position=c(0.3,0.8))+theme(axis.line.x = element_line(color="black", size = 1),axis.line.y = element_line(color="black", size = 0.5))
plot(p)
dev.off()
