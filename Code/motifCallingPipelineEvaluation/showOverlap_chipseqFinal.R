...
load("interimData/alldf1SubFam_0.35_chipseq.RDat")
a=alldf1SubFam
setkey(a,subFamily)
...
alltabl0=merge(a,b,all=TRUE,suffixes=c('.spec0.35','.spec0.7'))
...
cutoffs=paste("rank.",c('spec0.35','spec0.5','spec0.7','cutoff1e5'),sep="")
...
k=sum(alltablR[,!is.na(rank.spec0.35)])
q=sum(alltablR[,!is.na(rank.spec0.35) & rank.cutoff1e5<10])
pval0.35=1-phyper(q-1,m,n,k)
